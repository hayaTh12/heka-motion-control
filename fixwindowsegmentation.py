...
from loss.temporal_weighted_mse import TemporalWeightedMSE, TemporalWeightedMSEWithDecay 
...
class ModelBuilder:
    ...
    def get_criterion(self, weight=None):
        if self.loss == 'TemporalWeightedMSE':
            temporal_weights = self.config.get('temporal_weights', None)
            if temporal_weights is None:
                raise ValueError(
                    "Pour 'TemporalWeightedMSE', vous devez spécifier "
                    "'temporal_weights' dans la config"
                )
            horizon_size = self.config.get(
                'horizon_size',
                self.config['target_padding_length']
            )
            if len(temporal_weights) != horizon_size:
                raise ValueError(
                    f"Nombre de poids temporels ({len(temporal_weights)}) "
                    f"!= horizon_size ({horizon_size})"
                )

            normalize = self.config.get('normalize_temporal_weights', True)

            self.criterion = TemporalWeightedMSE(
                temporal_weights,
                normalize=normalize
            )
        ...
        elif self.loss == 'RMSE' and weight is not None:
            weight = torch.tensor(weight).to(self.device)
            self.criterion = WeightedRMSELoss(weight, mode="time")
