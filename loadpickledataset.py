...
class DataSet:
    ...
    def load_dataset(self):
        ...
        self._preprocess()
        self.x, self.y, self.labels = self.run_segmentation(self.x, self.y, self.labels)
    ...
    def run_predictive_horizon_segmentation(self, x, y, labels):
        """
        X_win = imu[t-W+1 : t+1]
        Y_hor = ik[t+d : t+d+H]
        labels_row = metadata at time t (anchor)
        Returns:
        X: (N, W, n_features)
        Y: (N, H, n_outputs)
        labels: DataFrame (N, n_cols)
        """
        W = int(self.config.get("window_size", self.config["target_padding_length"]))
        step = int(self.config.get("step_size", 5))
        d = int(self.config.get("delta_t_samples", 10))
        H = int(self.config.get("horizon_size", 20))

        X_all, Y_all, L_rows = [], [], []

        for imu_trial, ik_trial, lab_trial in zip(x, y, labels):
            T = len(imu_trial)
            ...
            for t in range(t_min, t_max + 1, step):
                x_win = imu_trial[t - W + 1 : t + 1]      # (W, n_features)
                y_hor = ik_trial[t + d : t + d + H]       # (H, n_outputs)

                # 1 label par fenêtre (anchor à t)
                l_row = lab_trial.iloc[t].to_dict()

                X_all.append(x_win)
                Y_all.append(y_hor)
                L_rows.append(l_row)

        self.x = np.asarray(X_all, dtype=np.float32)
        self.y = np.asarray(Y_all, dtype=np.float32)
        self.labels = pd.DataFrame(L_rows)

        return self.x, self.y, self.labels

    def run_segmentation(self, x, y, labels):
        if self.segmentation_method == 'predictive_horizon':
            self.x, self.y, self.labels = self.run_predictive_horizon_segmentation(x, y, labels)
        elif self.segmentation_method == 'fixedwindow':
